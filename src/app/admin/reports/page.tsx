...
import {
  weeklyRevenue, monthlyRevenue, peakHoursData, tableUtilization,
} from '@/data/mock-analytics';
import { formatCurrency } from '@/lib/formatters';
import { TrendingUp, BarChart3, PieChart as PieIcon, Calendar, ArrowDownToLine, Clock, Table2 } from 'lucide-react';
import { cn } from '@/lib/utils';

const dailyRevenue = [
  { date: '10 AM', revenue: 1200, orders: 4 },
  { date: '11 AM', revenue: 2800, orders: 8 },
  { date: '12 PM', revenue: 5400, orders: 16 },
  { date: '1 PM', revenue: 7200, orders: 22 },
  { date: '2 PM', revenue: 4800, orders: 14 },
  { date: '3 PM', revenue: 2100, orders: 6 },
  { date: '4 PM', revenue: 1500, orders: 5 },
  { date: '5 PM', revenue: 1800, orders: 6 },
  { date: '6 PM', revenue: 3200, orders: 10 },
  { date: '7 PM', revenue: 6800, orders: 20 },
  { date: '8 PM', revenue: 8900, orders: 26 },
  { date: '9 PM', revenue: 7600, orders: 22 },
  { date: '10 PM', revenue: 4200, orders: 12 },
];

const CHART_TOOLTIP_STYLE = {
  contentStyle: { backgroundColor: 'hsl(var(--surface-1))', borderColor: 'rgba(255, 255, 255, 0.05)', borderRadius: '12px', padding: '10px 14px', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' },
...
export default function AdminReportsPage() {
  ...
  const revenueData = activeTab === 'daily' ? dailyRevenue : activeTab === 'weekly' ? weeklyRevenue : monthlyRevenue;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/20 pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Sales velocity and peak dining analysis</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: `${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Revenue`, value: formatCurrency(totalRevenue), icon: TrendingUp, color: 'text-gold bg-gold/10 border-gold/20' },
          { label: 'Total Orders', value: totalOrders.toString(), icon: BarChart3, color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
          { label: 'Avg Order Value', value: formatCurrency(avgOrderValue), icon: PieIcon, color: 'text-emerald bg-emerald/10 border-emerald/20' },
        ].map((item, idx) => {
          ...
          <div className="h-[240px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={tableUtilization} margin={{ top: 5, right: 10, left: -25, bottom: 0 }}>
                <XAxis dataKey="tableNumber" stroke="hsl(var(--muted-foreground))" opacity={0.6} fontSize={9} tickFormatter={(v) => `T${v}`} axisLine={false} tickLine={false} dy={8} />
