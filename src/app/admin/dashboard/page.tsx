...
import { ShoppingBag, DollarSign, Clock, Armchair } from "lucide-react";
import {
  AreaChart,
  Area,
  XAxis,
...
export default function AdminDashboardPage() {
  ...
  const handleClearData = async () => {
    if (
      !confirm(
        "Clear all tables, menu, orders, and users? Dashboard will show zero.",
      )
    )
      return;
  ...
  const handleLoadSample = async () => {
    setDataAction("loading");
    try {
      await apiJson<{ seeded: boolean; message: string }>("/api/seed", {
        method: "POST",
      });
      await refreshAll();
    } finally {
  ...
  const peakHoursSlice = useMemo(
    () => peakHoursData.slice(2, 11),
    [peakHoursData],
  );
  ...
  const occupancyPct =
    tables.length > 0
      ? Math.round((occupiedTablesCount / tables.length) * 100)
      : 0;
